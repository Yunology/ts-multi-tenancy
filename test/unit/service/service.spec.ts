...
describe('Service class', () => {
  describe('Method config', () => {
    it('Default config', async () => {
      ...
      expect(instance.config<DefaultConfig>().num).to.be.eq(1);
    });
    ...
    it('Default config and overwrite with RuntimeTenant config', async () => {
      ...
      await instance.init(rt);
      expect(instance.config<OverwriteDefaultConfig>().num).to.be.eq(8);
    });
    ...
    it('Setup multiple default configs', async () => {
      ...
      await instance.init(rt);
      expect(instance.config<DefaultConfigOne>().foo).to.be.eq(1);
      expect(instance.config<DefaultConfigTwo>().bar).to.be.eq('a');
    });
    ...
    it('Empty default config and load RuntimeTenant config', async () => {
      ...
      await instance.init(rt);
      expect(instance.config<SomePredefineConfig>().bar).to.be.eq(123);
    });
  });
});
